...
bash: 					# Run bash inside `web` container
	docker compose exec $(DOCKER_EXEC_FLAGS) web bash

bash-root: 				# Run bash as root inside `web` container
	docker compose exec $(DOCKER_EXEC_FLAGS)u root web bash
...
dbshell: 				# Connect to database shell using `web` container
	docker compose exec $(DOCKER_EXEC_FLAGS) web python manage.py dbshell
...
lint:					# Run linter script
	docker compose exec $(DOCKER_EXEC_FLAGS) web /app/lint.sh
...
migrate:				# Execute Django migrations inside `web` container
	docker compose exec $(DOCKER_EXEC_FLAGS) web python manage.py migrate

migrations:				# Execute `makemigrations` inside `web` container
	docker compose exec $(DOCKER_EXEC_FLAGS) web python manage.py makemigrations

shell:					# Execute Django shell inside `web` container
	docker compose exec $(DOCKER_EXEC_FLAGS) web python manage.py shell
...
test:					# Execute `pytest` and coverage report inside `web` container
	docker compose exec $(DOCKER_EXEC_FLAGS) web bash -c 'coverage run -m pytest $(TEST_ARGS) && coverage report'
