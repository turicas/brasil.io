...
class ProcessNewSpreadsheetTaskTests(TestCase):
    ...
    @patch.object(StateSpreadsheet, 'check_is_ready_to_be_imported', Mock(side_effect=OnlyOneSpreadsheetException, autospec=True))
    ...
    def test_dispatch_new_spreadsheet_notification_if_nothing_to_compare_with(self, mock_notify):
        ...
        spreadsheet.check_is_ready_to_be_imported.assert_called_once_with()
        mock_notify.assert_called_once_with(spreadsheet)

    @patch.object(StateSpreadsheet, 'check_is_ready_to_be_imported', Mock(return_value=(False, ['errors']), autospec=True))
    ...
    @patch.object(StateSpreadsheet, 'check_is_ready_to_be_imported', Mock(return_value=(True, []), autospec=True))
    ...
    @patch.object(StateSpreadsheet, 'check_is_ready_to_be_imported', Mock())
    def test_do_not_process_spreadsheet_if_for_some_reason_it_is_cancelled(self):
        ...
        assert not spreadsheet.check_is_ready_to_be_imported.called
