...
def extract_ibge_data(schema_filename, population_filename):
    ...
    return rows.import_from_csv(population_filename, encoding="utf-8", force_types=schema)
...
def brazilian_cities_per_state(schema_filename="schema-population.csv", population_filename="population.csv"):
...
def get_city_info(city, state, schema_filename="schema-population.csv", population_filename="population.csv"):
    data = ibge_data_per_state(schema_filename, population_filename)
    try:
...
def get_state_info(state, schema_filename="schema-population.csv", population_filename="population.csv"):
    data = ibge_data_per_state(schema_filename, population_filename)
    try:
